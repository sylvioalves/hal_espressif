...
esp_err_t esp_rom_flash_read(size_t src_addr, void *dest, size_t size, bool allow_decrypt)
{
    ...
    while (remaining > 0) {
        copy_size = (remaining > FLASH_BLK_SZ) ? FLASH_BLK_SZ : remaining;
        aligned_size = (copy_size + src_offset + 3) & ~3; // Align to 4 bytes
        ...
        src_offset = 0;  // Only the first iteration needs the offset adjustment
    }
 ...
 esp_err_t esp_rom_flash_write(size_t dest_addr, void *src, size_t size, bool write_encrypted)
{
    ...
    uint32_t temp_buf[FLASH_BLK_SZ];
    ...
    while (remaining > 0) {
        size_t copy_size = (remaining > FLASH_BLK_SZ) ? FLASH_BLK_SZ : remaining;
        size_t aligned_size = (copy_size + dest_offset + (alignment - 1)) & ~(alignment - 1); // Align to required boundary
        ...
        src_ptr += copy_size;
        start_addr += aligned_size;
        dest_offset = 0;  // Only the first iteration needs the offset adjustment
    }
